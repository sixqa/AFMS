#!/usr/bin/env python3
"""
train_det.py — 检测模型统一训练入口

三种调用方式：
    python tools/train_det.py --yaml configs/shrimp.yaml   # 用户添加新物种：传 YAML 文件
    python tools/train_det.py --species carp               # 快捷方式：直接选物种
    python tools/train_det.py --config path/to/config.py   # 高级：自定义配置文件
...
def get_available_configs() -> list:
    """扫描 configs/det/ 获取所有配置文件"""
    cfg_dir = Path(__file__).parent.parent / "configs" / "det"
...
def list_species_and_exit():
    ...
    for s in configs:
        print(f"  - {s}")
    print()
    print("用法: python tools/train_det.py --yaml configs/shrimp.yaml")
    print("  或: python tools/train_det.py --species 物种名")
    print("  或: python tools/train_det.py --config path/to/config.py")
    sys.exit(0)


def parse_args():
    p = argparse.ArgumentParser(
        description="AFMS 检测模型训练",
        formatter_class=argparse.RawDescriptionHelpFormatter)
    g = p.add_mutually_exclusive_group()
    g.add_argument("--yaml", help="物种 YAML 配置文件路径（推荐）")
    g.add_argument("--config", help="配置文件路径")
    g.add_argument("--species", "-s",
                   help="物种名，自动读取 configs/det/{物种}.py")
...
def resolve_config(args) -> Path:
    ...
    if args.yaml:
        ...
        cfg = Path(__file__).parent.parent / "configs" / "det" / f"{species_name}.py"
        if not cfg.exists():
        ...
        return cfg
    # --species 模式
    cfg = Path(__file__).parent.parent / "configs" / "det" / f"{args.species}.py"
    if not cfg.exists():
        print(f"[ERROR] 配置文件不存在: {cfg}")
...
def fix_paths(cfg, trainer_root: Path):
    """将配置文件中的相对路径（../datasets/xxx）转为绝对路径"""
    import re
...
def main():
    ...
    wd = Path(args.work_dir or f"runs/det_{species_name}_{ts}").resolve()
    wd.mkdir(parents=True, exist_ok=True)

    print("=" * 60)
    print(f"  AFMS Detection Trainer")
    ...
    if not args.no_archive:
        archive = trainer_root / f"models/{species_name}/{variant}/detection"
        archive.mkdir(parents=True, exist_ok=True)
        for pth in wd.glob("best_*.pth"):
            shutil.copy2(str(pth), str(archive / pth.name))
        shutil.copy2(str(cfg_path), str(archive / f"{species_name}_det.py"))
