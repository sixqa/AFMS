#!/usr/bin/env python3
"""
evaluate_det.py — 检测模型统一评估

用法：
    # 从训练归档评估
    python tools/evaluate_det.py --species carp --variant v20250502

    # 直接指定配置和权重
    python tools/evaluate_det.py --config runs/det_carp_xxx/carp_det.py \\
                                 --checkpoint runs/det_carp_xxx/best_coco_bbox_mAP_xxx.pth
"""
...
def parse_args():
    p = argparse.ArgumentParser(description="AFMS 检测模型评估")
...
def main():
    ...
    if args.config and args.checkpoint:
        ...
        if args.checkpoint:
            ...
            config_path = trainer_root / f"configs/det/{args.species}.py"
            ckpt_path = Path(args.checkpoint)
        else:
            # 从 models/ 目录自动查找配置和最佳权重
            model_dir = trainer_root / f"models/{args.species}/{variant}/detection"
            if not model_dir.exists():
                print(f"[ERROR] 评估目录不存在: {model_dir}")
                print(f"       请先训练: python tools/train_det.py --species {args.species}")
                sys.exit(1)
    ...
    print(f"评估 {config_path.stem} 检测模型...")
    ...
    ts = datetime.now().strftime("%Y%m%d_%H%M%S")
    report = f"# Detection Report: {config_path.stem}\n\n"
    ...
    report_path = evals_dir / f"{config_path.stem}_det_report.html"
    html_content = f'<!DOCTYPE html><html><head><meta charset="utf-8"><title>{config_path.stem} DET Report</title></head><body><pre>{report}</pre></body></html>'
